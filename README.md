...
- **Каталог:** единая сущность `content_item` для фильмов, сериалов, эпизодов и телеканалов; вложенность задаётся через `parent_content_id`.
...
- видео и постеры вынесены в файловые/object storage
...
Для `recommendation_cache`, `content_card_cache`, `playback_checkpoint_buffer` и `cdn_segment_cache` цифры получены инженерным разложением уже посчитанных нагрузок из части 2, потому что выше они есть как функции продукта, но не выделены отдельными строками RPS.
...
|---|---|---|---:|---:|---|---|
| `user_account` | Профиль пользователя. Ключ: `user_id` | `~1 KB/row`, всего `~60 GB` | `~50` | `~5` | **Strong** | Ключи распределяются почти равномерно по `user_id`, горячие записи возникают только у активных пользователей |
| `user_settings` | Настройки воспроизведения и UX. Ключ: `user_id` | `~1 KB/row`, всего `~60 GB` | `~50` | `~5` | **Strong**, read-after-write для владельца | Хорошее распределение по `user_id`, конфликтов мало |
| `auth_session` | Refresh-токены, устройства, активные сессии. Ключ: `session_id`, secondary по `user_id` | `~1 KB/row`, `2-3` сессии на пользователя, всего `~120-180 GB` | `~22` | `~22` | **Strong** | Нагрузка концентрируется на активной аудитории; удобнее распределять по `user_id` или `session_id` с локальностью по пользователю |
| `content_item` | Каноническая запись каталога для фильма / сериала / эпизода / канала. Ключ: `content_id` | `~10 KB/row`, `~80k` записей, всего `~0.8 GB` | `~900` | `<1` | **Strong** на master-данных, eventual в производных индексах | Сильный перекос по популярным тайтлам и премьерам; горячие `content_id` должны уходить в кэш |
| `watchlist_item` | Список «Буду смотреть». Ключ: `(user_id, content_id)` | `~64 B/row`, `~30` записей на пользователя, всего `~115 GB` | `~200` | `~44` | **Strong**, read-after-write для владельца на всех устройствах | Естественный ключ распределяется по `user_id`; перекос умеренный, hot keys почти отсутствуют |
| `watch_progress` | Последний сохранённый прогресс и история просмотров. Ключ: `(user_id, content_id, device_id)` | `~64 B/row`, `~200` записей на пользователя, всего `~768 GB` | `~222` | `~2200` после коалессации буфером | **Per-user monotonic**, read-after-write для владельца; межрегионально можно допустить eventual в пределах секунд | Основной partition key — `user_id`; запись массовая, но распределена по DAU достаточно равномерно |
...
| `search_index` | Полнотекстовый индекс каталога. Ключ: `doc_id`, secondary по токенам / n-gram | всего `~2 GB` | `~444` | `<1` | **Eventual** относительно `content_item` | Нагрузка неравномерна: горячие префиксы, имена франшиз, новые релизы |
| `recommendation_cache` | Предрасчитанные персональные рекомендации. Ключ: `(user_id, rank)` | `~100` элементов на активного смотрящего пользователя, всего `~60 GB` | `~1300` | `~50` | **Eventual**; для пользователя важна свежесть в пределах минут/часов, не строгая консистентность | Распределение по `user_id` близко к DAU; горячих одиночных ключей почти нет |
| `content_card_cache` | Кэш сериализованных карточек каталога / главной. Ключ: `content_id` | `~2 KB/card`, полный каталог `<1 GB` | `~4444` | `~5` | **Eventual** с инвалидацией по `content_id` | Сильный Zipf-перекос по популярным тайтлам; именно этот кэш снимает основной browse-read трафик с `content_item` |
| `poster_image_store` | Origin-хранилище постеров и изображений. Ключ: `object_key` | `0.5-1 MB/object`, всего `~40-80 GB` | `<=50` на origin | `<1` | **Strong** на origin-метаданных, eventual на раздаче через `img`/CDN | Hot set определяется популярными тайтлами; большая часть чтений должна отдаваться из edge-кэша |
| `media_package_store` | Origin-хранилище видео: manifests и segments ABR-профилей. Ключ: `object_key` | `8.7-17.4 GB` на единицу контента, всего `~697 TB - 1.39 PB` | `~900` на origin | `<<1` | **Strong** на origin, eventual при распространении в edge | Крайне сильный перекос по популярным тайтлам, первым сегментам и средним bitrate-профилям |
| `playback_checkpoint_buffer` | Быстрый буфер чекпоинтов прогресса перед записью в durable store. Ключ: `(user_id, content_id, device_id)` | `~128 B/key`, при `400k-800k` активных просмотров `~50-100 MB` | `~222` | `~26 667` | **In-memory monotonic merge**, допускается потеря части последних секунд при аварии, но не backward jump прогресса | Нагрузка широкая и хорошо распределена по активным пользователям; нужен merge по ключу вместо append-only записи |
| `cdn_segment_cache` | Региональный CDN-кэш видео-сегментов и manifests. Ключ: `(region_id, object_key)` | `~5 TB/edge-node`, порядка `~30-60 TB` на регион | `~133 333` | `~700` на заполнение при miss | **Eventual**, TTL-based | Самый сильный Zipf-перекос во всей системе: горячие первые сегменты, популярные тайтлы и несколько bitrate-профилей дают основной трафик |
