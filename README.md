...
Ниже используются только сущности, которые прямо следуют из `README.md`, без копирования структуры из `EXAMPLE.md`:

- **Пользовательский контур:** пользователь, настройки, сессии/устройства.
...
Отдельную сущность `WatchHistory` в этой логической схеме не выделяю: для продуктового сценария Кинопоиска достаточно хранить актуальный прогресс в `WatchProgress`, а полную событийную историю просмотра при необходимости логичнее уводить в отдельный аналитический/event-контур.

Такой набор нужен именно для моей модели Кинопоиска:
...
- видео и постеры вынесены в файловые/object storage, потому что в `README.md` отдельно посчитаны `img` и `cdn` домены, а также объёмы origin-хранилищ.

### Логическая схема

Исходник схемы draw.io: [images/part5-logical-schema.drawio](images/part5-logical-schema.drawio)

### Таблица сущностей и хранилищ

Все значения ниже — **пиковые оценки** для логической схемы.  
Для `RecommendationCache`, `ContentCardCache`, `PlaybackCheckpointBuffer` и `CdnSegmentCache` цифры получены инженерным разложением уже посчитанных нагрузок из части 2, потому что в `README.md` они есть как функции продукта, но не выделены отдельными строками RPS.
...
| `AuthSession` | Refresh-токены, устройства, активные сессии. Ключ: `session_id`, secondary по `user_id` | `~1 KB/row`, `2-3` сессии на пользователя, всего `~120-180 GB` | `~22` | `~22` | **Strong** | Нагрузка концентрируется на активной аудитории; удобнее распределять по `user_id` или `session_id` с локальностью по пользователю |
...
| `RecommendationCache` | Предрасчитанные персональные рекомендации. Ключ: `(user_id, rank)` | `~100` элементов на активного смотрящего пользователя, всего `~60 GB` | `~1300` | `~50` | **Eventual**; для пользователя важна свежесть в пределах минут/часов, не строгая консистентность | Распределение по `user_id` близко к DAU; горячих одиночных ключей почти нет |
...
Ключевые решения для этой предметной области:

1. **Разделение canonical и derived stores**  
   Каталог (`ContentItem`) остаётся небольшим canonical-слоем, а поиск, карточки каталога и рекомендации вынесены в отдельные производные хранилища.

2. **Буферизация прогресса просмотра**  
   Без `PlaybackCheckpointBuffer` запись `~26.7k` peak QPS попадёт напрямую в durable-хранилище и станет главным узким местом пользовательского контура.

3. **Разделение origin и edge для медиа**  
   `PosterImageStore` и `MediaPackageStore` хранят origin-данные, а `CdnSegmentCache` принимает на себя основной read-трафик, что соответствует уже выбранному доменному разбиению `img` / `cdn`.

4. **Распределение по двум типам ключей**  
   Пользовательские таблицы естественно раскладываются по `user_id`, а каталог и медиа — по `content_id`/`object_key`; это отражает реальную структуру нагрузки из частей 2-4.
