...
Для формирования персональных рекомендаций выбран гибридный batch / nearline-подход.  
Его основа — комбинация контентной близости тайтлов, истории просмотров пользователя, его оценок и дополнительных поведенческих сигналов.

Упрощённо итоговый скор можно представить как:

`final_score = alpha * content_score + beta * behaviour_score + gamma * popularity_score`

где:
- `content_score` — близость контента к уже просмотренным и оценённым тайтлам;
- `behaviour_score` — вес пользовательских действий: просмотров, досмотров, оценок, поисков;
- `popularity_score` — поправка на общий интерес аудитории к тайтлу.

События просмотров, поисков, оценок и взаимодействий с карточками контента сначала отправляются в `Kafka`, после чего сохраняются в `ClickHouse`. Далее recommendation worker периодически пересчитывает рекомендации для пользователя и формирует готовый `top-N` список. Итоговая выдача сохраняется в `RecommendationCache`, откуда уже читается приложением.

Такой подход выбран вместо online-пересчёта рекомендаций при каждом открытии главной страницы. Это снижает нагрузку на `PostgreSQL`, позволяет держать стабильное время ответа и хорошо подходит для read-heavy сценария персональной выдачи.

### 7.2 Сохранение прогресса просмотра

Во время воспроизведения клиент раз в 30 секунд отправляет checkpoint с текущей позицией просмотра.  
Если писать каждый такой checkpoint напрямую в основную БД, это создаст слишком большую write-нагрузку на транзакционный контур.

Поэтому используется двухступенчатая схема:
1. последний checkpoint по ключу `(user_id, content_id, device_id)` сначала сохраняется в `PlaybackCheckpointBuffer`;
2. в буфере хранится только наиболее актуальный прогресс;
3. затем прогресс периодически или при завершении просмотра сбрасывается в durable-хранилище `WatchProgress`.

Такая схема уменьшает число мелких write-запросов в `PostgreSQL`, но при этом сохраняет основной пользовательский сценарий — возможность продолжить просмотр с нужного места на другом устройстве. Допускается потеря нескольких последних секунд прогресса при аварии, но не откат назад уже просмотренной позиции.

### 7.3 Раздача видео через CDN и adaptive bitrate

Основной объём трафика в онлайн-кинотеатре создаёт именно видеоконтент, поэтому его нельзя раздавать напрямую из backend и основной БД.

Видео хранится в `MediaPackageStore` в виде manifests и сегментов нескольких bitrate-профилей.  
Клиент сначала получает manifest, после чего выбирает подходящий bitrate в зависимости от качества сети и начинает запрашивать сегменты. Основная часть сегментов отдаётся через `CDN`, а origin-хранилище используется только как источник при cache miss.

Такой подход решает сразу две задачи:
- уменьшает нагрузку на центральное origin-хранилище;
- позволяет поддерживать стабильное воспроизведение даже при нестабильном соединении пользователя.

Поэтому в архитектуре сервиса видеоконтент физически отделён от OLTP-контура и обслуживается отдельным медиа-контуром через `S3 / MinIO` и `CDN`.
