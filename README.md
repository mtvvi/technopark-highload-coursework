...
- **L7** выполняет **SSL termination** и HTTP-маршрутизацию; возможная реализация — **NGINX / NGINX Ingress Controller**; резервирование — **N + 1**.

Для **CDN** используется отдельный контур:

`Клиент → локальное распределение → CDN cache node → origin (при cache miss)`

Это связано с тем, что для API и web-трафика важны SSL termination и L7-маршрутизация, а для видеотрафика CDN основным ограничителем является сетевая пропускная способность.
...
Итого для московского ДЦ:

- **пиковый API RPS:** `~14 333`
- **пиковая API-полоса:** `~0.92 Гбит/с`

### Выбор производительности одного L7-узла

В качестве ориентира используем результаты тестирования **NGINX Ingress Controller for Kubernetes** [^8]. Для сервера уровня **16 CPU** в тестах были получены следующие значения:
