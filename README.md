...
|---|---|---|
| `NGINX` | L7-балансировка и reverse proxy | Используется для SSL termination, маршрутизации HTTP-запросов и балансировки трафика между backend-сервисами |
...
| `PostgreSQL` | основная OLTP-БД | Подходит для транзакционных данных: пользователи, каталог, избранное, прогресс, оценки, отзывы; нужен там, где важны ACID и строгая консистентность |
| `Redis Cluster` | кэши и буферы | Используется для быстрого доступа к горячим данным и для `PlaybackCheckpointBuffer`, где критична минимальная задержка |
| `OpenSearch / Elasticsearch` | полнотекстовый поиск | Отдельный поисковый движок лучше подходит для поиска по названиям, франшизам и персонам, чем запросы через `LIKE` в `PostgreSQL` |
| `Kafka` | асинхронная шина событий | Развязывает online-контур и аналитические процессы: события просмотров, поисков и оценок обрабатываются асинхронно |
| `ClickHouse` | аналитика и подготовка рекомендаций | Подходит для хранения сырых событий, расчёта агрегатов и подготовки данных для рекомендательной системы |
| `S3 / MinIO` | origin-хранилище медиаобъектов | Используется для хранения постеров, manifests и сегментов видео как тяжёлых бинарных объектов |
| `CDN` | edge-раздача видео и изображений | Позволяет отдавать тяжёлый медиаконтент ближе к пользователю, снижает задержку старта видео и разгружает origin |

## Часть 9. Обеспечение надёжности

| Компонент системы | Способ резервирования / отказоустойчивости | Что это даёт |
|---|---|---|
| **Глобальная маршрутизация (`www`, `api`, `img`, `static`)** | `Weighted GSLB` + `health checks` + короткий TTL + failover в соседний регион | При деградации или отказе дата-центра новые клиенты автоматически перестают направляться в него, а трафик уходит в ближайший рабочий регион |
| **Глобальная маршрутизация видеотрафика (`cdn`)** | `Anycast` / `GeoDNS` для edge-узлов CDN | Пользователь получает видео с ближайшей точки присутствия; отказ одного региона не делает весь видеоконтур недоступным |
...
| **Backend API и stateless-сервисы** | Несколько экземпляров сервиса за L7-балансировщиком; межрегиональный failover через GSLB | Отказ одного приложения или одного сервера не делает функцию сервиса полностью недоступной |
| **`PostgreSQL`** | `primary + replica`, `PgBouncer`, read/write split; резервное копирование: ежедневный full backup + WAL-архивирование + `PITR` | Снимается единая точка отказа в online-чтении, а при логической ошибке или аварии возможен point-in-time restore |
| **`Redis Cluster` / `PlaybackCheckpointBuffer`** | Кластерный режим; данные буфера не считаются критичными; потеря части checkpoint-ов допустима | Кэш и буфер не превращаются в жёсткую зависимость системы; при сбое возможна потеря нескольких последних секунд прогресса, но durable-состояние хранится в `WatchProgress` |
...
| **`Kafka`** | Кластер из нескольких brokers, репликация партиций, producer `acks=all`, consumer group rebalance | Отказ одного broker-а не теряет весь event stream и не останавливает полностью асинхронный контур |
| **`ClickHouse`** | Replicated tables / репликация между узлами + retention policy + backup replicated tables | Аналитика и рекомендации не зависят от одного узла; данные событий можно восстановить после аппаратного сбоя |
...
| **`CDN / edge cache`** | Не бэкапится как durable-хранилище; полностью восстанавливается из `S3 / MinIO` origin | Edge-кэш можно считать расходуемым слоем: его потеря ухудшает latency, но не уничтожает данные |
| **Мониторинг и health checks** | Активные проверки доступности, метрики, алерты на ключевые контуры | Система может не только переживать отказ, но и быстро обнаруживать деградацию до полного инцидента |
| **Graceful degradation** | Для некритичных функций используются fallback-сценарии | При отказе рекомендаций можно показывать “популярное” или “новинки”; при отказе поиска — жанры и каталог; при недоступности отзывов — карточку без блока рецензий; при сбое изображений — заглушку |
