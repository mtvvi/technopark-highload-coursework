...
- в качестве ориентира по ABR-профилям используем пример HLS bitrate ladder из AWS (по Apple HLS Authoring Spec):  
  `7.8, 6, 4.5, 3, 2, 1.1, 0.8, 0.4, 0.2 Mbps` [^5]

Суммарный битрейт набора ABR-профилей (по перечисленным профилям):
- `Bsum = 7.8 + 6 + 4.5 + 3 + 2 + 1.1 + 0.8 + 0.4 + 0.2 = 25.8 Мбит/с`

> В той же статье в примере manifest указаны значения `BANDWIDTH`, сумма которых составляет `30 968 350 бит/с ≈ 30.97 Мбит/с`. Для расчёта ниже используется более стандартное значение `25.8 Мбит/с` по bitrate ladder. [^5]

Размер одной единицы контента:
- при 45 мин:
  - `Sunit_video_45 = 25.8 Мбит/с * 2700 с / 8 ≈ 8.71 ГБ`
- при 90 мин:
  - `Sunit_video_90 = 25.8 Мбит/с * 5400 с / 8 ≈ 17.42 ГБ`
...
| Прогресс просмотра | 120 | 1 152 000 000 | 13 333.33 | 26 666.67 |
